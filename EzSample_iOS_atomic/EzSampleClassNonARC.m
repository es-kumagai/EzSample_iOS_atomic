...
- (void)dealloc
{
	[_valueForReplaceByAtomic release];
	[_valueForReplaceByNonAtomic release];
	[_valueForReplaceByAtomicReadAndNonAtomicWrite release];
	
	_valueForReplaceByAtomic = nil;
	_valueForReplaceByNonAtomic = nil;
	_valueForReplaceByAtomicReadAndNonAtomicWrite = nil;
...
- (void)testForAtomicReadAndNonAtomicWriteWithOutput:(BOOL)output outputFormat:(NSString*)outputFormat
{
	@autoreleasepool
	{
		// ローカルスコープを定義します。
		{
			...
			if (output) NSLog(outputFormat, @"Property will write.");
			_valueForReplaceByAtomicReadAndNonAtomicWrite = [value retain];
			if (output) NSLog(outputFormat, @"Property did write.");
